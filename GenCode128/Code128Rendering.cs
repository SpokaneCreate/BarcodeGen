...
namespace GenCode128
{
	...
	{
      public static double wScale = 0.7;
      public static double hScale = 0.7;
      ...
      public static Image MakeBarcodeImage( string InputData, double BarWeight, bool AddQuietZone )
      {
         ...
         // width = (int) ((((codes.Length - 3) * 11 + 35) * BarWeight)*wScale);
         // height = Convert.ToInt32( System.Math.Ceiling( Convert.ToSingle(width) * .05F) );

         width = (int)(( (codes.Length - 3) * BarWeight) * wScale);
         height = (int) hScale;

         if (AddQuietZone)
         {
            width += (int) (2 * cQuietWidth * BarWeight);  // on both sides
         }
         ...
         using (Graphics gr = Graphics.FromImage(myimg))
         {
            ...
            // skip quiet zone
            int cursor = (int) (AddQuietZone ? cQuietWidth * BarWeight : 0);
